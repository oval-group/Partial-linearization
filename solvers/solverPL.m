function [model, progress] = solverPLFW(param, options)
...
%     model.w = sparse(d,1);
%     model.w_cs = sparse(d,1);
%     %wMat = sparse(d,n);
% else
%     model.w = zeros(d,1);
%     model.w_cs = zeros(d,1);
%     %wMat = zeros(d,n);
% end
...
for p=1:options.num_passes
    ...
    for i = 1:n
        ...
        if options.useRAM
            ...
            save( sprintf(param.belS_fileFormat,i), 'nodeBelS', 'edgeBelS');
    ...
    if options.useRAM
        for i = 1:param.numTrain
            ...
            %edgeBelAlphaPrev{i} = (1-gamma)*edgeBelAlphaPrev{i} + gamma*edgeBelS{i};
        end
    else
        for i = 1:param.numTrain
            % load previous and S beliefs
            load( sprintf(param.belPrev_fileFormat,i ) );
            load( sprintf(param.belS_fileFormat,i) );
    ...
    if (options.debug)
        ...
        fprintf('pass %d (iteration %d), SVM primal = %f, SVM dual = %f, duality gap = %f, train_error = %f, gamma = %.3f \n', ...
            p, k+1, primal, f, gap, train_error, gamma);
        ...
        %progress.expt_diff_max = [progress.expt_diff_max; expt_diff_max];
    ...
    f = -objective_function(model.w, model.ell, lambda);
